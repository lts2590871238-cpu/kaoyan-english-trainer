...
if set(senses)!=set(terms): errors.append('local sense table is not a 1:1 mirror of the 3000 terms')
badlocal=[]
for x in lex:
    if not x.get('dict_zh') or (x.get('type')=='word' and not x.get('definition_en')): badlocal.append(x['term'])
if badlocal: errors.append(f'local dictionary incomplete before AI: {badlocal[:20]}')
broken=[]; debt=[]
for x in lex:
    if not x.get('contexts'):
        if not x.get('needs_context_fill'): broken.append(x['term']+'(unmarked)')
        else: debt.append(x['term'])
...
print(f'3000 unique canonical learning items; local bilingual dictionary complete; 100x30 schedule exact; practice-context queue={len(debt)}.')
