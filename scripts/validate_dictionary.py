...
for x in lex:
    ...
    if not str(s.get('definition_en') or x.get('definition_en') or '').strip(): miss.append('definition_en')
    if not str(s.get('pos') or x.get('pos') or '').strip(): miss.append('pos')
...
print(f'3000/3000 learning items have Chinese meaning + English definition + POS; lookup entries={len(lookup)}.')
